...
void cityBlock(pcl::visualization::PCLVisualizer::Ptr& viewer, ProcessPointClouds<pcl::PointXYZI>* pointProcessorI, const pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud) 
{
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr filterCloud 
        = pointProcessorI->FilterCloud(cloud, 0.1, Eigen::Vector4f(-20, -6, -5, 1), Eigen::Vector4f(30, 7, 2, 1));
  std::pair<pcl::PointCloud<pcl::PointXYZI>::Ptr, pcl::PointCloud<pcl::PointXYZI>::Ptr> segmentCloud = pointProcessorI ->SegmentPlane(filterCloud, 100, 0.2);
      // cluster obstacles
    std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> cloudClusters = pointProcessorI->Clustering(segmentCloud.first,  1.0, 3, 500);
        renderPointCloud(viewer, segmentCloud.second, "plane", Color(0,1,0));
    int clusterId = 0;
    for (pcl::PointCloud<pcl::PointXYZI>::Ptr cluster: cloudClusters)
    {
        renderPointCloud(viewer, cluster, "obstacle "+std::to_string(clusterId), Color(0,0,1));
